...
def main():
    ...
    #If consent is given and the subject gives confirmation of understanding, then the experiment can be run
        ...
        #Human ID given
            ...
            csv_filename = "human_test_data"

        #First open the file with all the previously gathered data
        try:
            dataframe_Tau = pd.read_csv(f"../{csv_filename}_Tau.csv", header=0)
        ...
        dataframe_Tau.to_csv(f"../{csv_filename}_Tau.csv", index=False)
        ...
        dataframe_WSLS.to_csv(f"../{csv_filename}_WSLS.csv", index=False)
